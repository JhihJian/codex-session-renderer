...
- 页面默认进入阅读视图，并提供四种主视图：
  ...
  - 原始事件视图：负责诊断，保留原始事件查看能力；它把会话事件摘要提升为主视图，左侧按事件索引和分类浏览，右侧显示选中事件的结构化摘要和 Pretty JSON，事件列表与预览区各自独立滚动；当选中上下文压缩事件时，会优先展示压缩摘要正文、窗口信息、替换历史数量和被替换对话预览。详情受限时，同一视图改为有界诊断，不展示或累计完整 JSONL：列表只含服务端当前页摘要，前进以 `nextCursor` 获取下一页，返回只访问最多 6 页本地历史；服务端 `snapshot` 令牌必须随续页和单事件读取带回，当前诊断代际的任一入口变化后返回 `409/session_snapshot_changed`，页面清空过期摘要、选择和完整事件缓存后提供重新开始入口。重新开始会取消该代际的分页和单事件读取；迟到的旧代际响应不能影响新摘要。读取到字节边界时会停止翻页并明确标记未读取的尾部。完整原始内容仍通过单事件接口按需读取。
...
- `snapshot`：分页续页和单事件读取时带回摘要页的 `page.snapshot`。令牌绑定数据源版本和文件签名，不含路径或正文；不匹配时返回 `409`，错误详情 `code` 为 `session_snapshot_changed`，调用方必须清空旧摘要、当前选择和已缓存完整事件后从头开始。
