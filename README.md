...
- 正文还显示有持久化证据的上下文事件：Codex/Pi 的压缩时间和摘要、Pi 会话中的 Skill 指令块，以及 `read` 读取 `SKILL.md` 的记录与工具结果。启动时的系统提示、可用工具或 Skills 索引未写入 JSONL，不会由当前磁盘状态反推为历史事件。
...
- 桌面正文右侧提供极简轮次导航。节点定位轮次，琥珀细线标记本轮压缩，青绿细线标记内嵌子代理调用；它不复制执行细节，移动端不显示该导航。
