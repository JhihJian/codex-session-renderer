...
归档接口为 `GET /api/sources/:sourceId/prompts`，支持 `scope=recent24h|history|all`、`q`、`project`、`status` 和 `limit` 参数。页面会复用左侧时间分类：实时/近一天读取 `recent24h`，更早读取 `history`；同一会话按文件路径、大小、修改时间和更新时间做增量缓存，未变化的会话不会重复解析 JSONL。归档不写入 Codex 文件或新数据库；正文未同步、仅有图片附件、没有有效用户输入和读取失败都会明确标记。远端历史索引只有元数据，不能生成虚假的首个提示词；需要先把正文同步到本机快照。
