...
命中详情边界时详情接口仍返回 `200`，但含 `complete: false` 与机器可识别的 `readState.code = session_read_limited`（或文件变化时 `session_file_changed`），`turns/events/compact/trace/audit` 都为空值，前端不会把部分内容当作完整会话。工作台的“原始事件”会切换为明确标记的有界诊断，保留当前会话和数据源语境；它只按页读取事件摘要，最多保留当前会话、来源和快照下最近 6 页本地历史。前进或后退命中这段历史时只切换缓存页，不重复请求或追加页面；只有未缓存的后页才使用服务端游标读取。完整原文仅在复核台“来源”页按需读取，并携带当前页快照令牌校验仍是同一文件版本。浏览器端完整事件缓存与摘要页独立，使用 LRU，最多保留 24 条且 UTF-8 JSON 估算总量最多 8 MiB；命中会刷新访问顺序，单条超过 8 MiB 只用于当前展示、不写入缓存。摘要页超过 6 页淘汰时，已不属于保留摘要页的完整事件也会同步释放，因此跨大量诊断页展开来源不会累积浏览器内存；被淘汰内容再次打开会重新读取当前快照。分页续页的旧令牌在请求开始前不匹配时返回 `409/session_snapshot_changed`；单条来源除同一前置校验外，还会在读取后复核文件签名，读取期间变化则返回 `409/session_file_changed`。两者都表示当前诊断版本已失效：仅当前诊断代际、当前来源与会话、且请求快照仍等于当前快照时，任一入口收到这两个结构化响应之一才会清空过期摘要、当前事件选择和完整事件缓存，并显示可恢复的“重新开始读取”回执；普通读取失败与用户取消仍保留各自原有状态。显式重新开始会取消在途分页和单事件读取，并替换诊断代际；即使取消竞争下旧快照的迟到响应抵达，也不能清空已读到的新快照。工作台实际显示的每条摘要都可按需回读；到达该上限会停止翻页，直接越界请求返回 `413/session_event_scan_limited`。切换视图、会话、数据源或刷新也会取消诊断及单事件请求，文件变化、当前快照变化和网络失败会清空旧页后提示重新开始，绝不混合不同文件版本的内容。`/markdown` 则返回 `413` JSON 状态，绝不导出部分 Markdown。原始事件分页接口仍按请求的 `limit` / `maxScan` 做流式诊断，并受上述单次读取字节边界和服务并发闸门保护。
...
- **会话**默认在标题下显示只读派生的交接摘要，依次给出目标、当前状态、结果、验证、风险/缺口、改动范围和子代理。摘要只复用已加载的详情、Audit、Trace 和变更统计；详情受读取上限保护时会明确显示哪些结论无法派生，不会用部分数据伪造完成状态。
...
- Chromium 契约覆盖桌面三栏与复核台键盘调整、窄屏流式复核区和移动三面板；真实 tab/panel 的双向 ARIA 关联、筛选/模块切换的 radio/toolbar 语义、移动面板非当前项不可聚焦；移动端任务归档复核状态及退出恢复；本机 Codex 空列表时对 Pi 可读会话的有界发现和显式切换；延迟加载的 `aria-busy`、持久成功/失败状态和取消/过期响应不公告；隐藏面板的焦点迁移和设置 dialog 的 Escape/关闭按钮焦点恢复；任务归档加载取消；远端历史索引的真实分页范围、末页、页缓存回退和搜索取消；远端“近一天/更早”索引第 2 页成功或失败拉取快照后不退回 `sessions`、重建/保留分页令牌；以及受限详情的工作台内有界诊断分页、单事件展开的快照变化清理/恢复、普通失败保留与离开诊断后的取消/过期响应隔离。
...
- 页面默认进入阅读视图，并提供四种主视图：
  ...
  - 原始事件视图：负责诊断，保留原始事件查看能力；它把会话事件摘要提升为主视图，左侧按事件索引和分类浏览，右侧显示选中事件的结构化摘要和 Pretty JSON，事件列表与预览区各自独立滚动；当选中上下文压缩事件时，会优先展示压缩摘要正文、窗口信息、替换历史数量和被替换对话预览。详情受限时，同一视图改为有界诊断，不展示或累计完整 JSONL：列表只含服务端当前页摘要，前进以 `nextCursor` 获取下一页，返回只访问最多 6 页本地历史；服务端 `snapshot` 令牌必须随续页和单事件读取带回。旧令牌不匹配返回 `409/session_snapshot_changed`，单事件读取期间文件变化返回 `409/session_file_changed`；当前诊断代际的任一入口收到其中任一种响应，页面都会清空过期摘要、选择和完整事件缓存后提供重新开始入口。复制 JSON 遇到这两类响应会额外给出明确的复制失败回执，绝不把重置后的读取误报为复制成功。重新开始会取消该代际的分页和单事件读取；来源展开、复制和移动端离开复核台都会取消或失效旧请求，迟到的旧代际响应不能影响新摘要、来源预览或复制回执。读取到字节边界时会停止翻页并明确标记未读取的尾部。完整原始内容仍通过单事件接口按需读取。
