...
归档接口为 `GET /api/sources/:sourceId/prompts`，支持 `q`、`project`、`status` 和 `limit` 参数。归档不写入 Codex 文件或新数据库；正文未同步、仅有图片附件、没有有效用户输入和读取失败都会明确标记。远端历史索引只有元数据，不能生成虚假的首个提示词；需要先把正文同步到本机快照。
