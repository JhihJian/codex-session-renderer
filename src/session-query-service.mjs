...
import { readJsonl, readJsonlLineWithDiagnostics, readJsonlRange, readJsonlWithDiagnostics } from "./jsonl-reader.mjs";
import {
  buildTrace,
  buildTurns,
  compactChildBase,
...
import { createPiGoalMessageProjector, isLikelyCodexGoalControlText } from "./pi-goal-projection.mjs";
import { createAbortError, createConcurrencyGate, createSessionDetailCoordinator, fileSignature, isAbortError } from "./session-detail-coordinator.mjs";
import {
...
export function createSessionQueryService() {
  ...
  async function getSessionDetail(context, id, options = {}) {
    ...
    return { ...result.value, related: await getSessionLineage(context, session, options) };
  }
  ...
  async function querySessionEvents(context, id, params, projectionOptions = {}, options = {}) {
    ...
    const sessionWithStat = withFileStat(session, beforeStat);
