...
test("Harness 页面以验证关口组织队列，并按需展示技术证据", async ({ page }) => {
  ...
  await expect(page.locator(".queue-row").filter({ hasText: "3 条同源记录" })).toHaveCount(1);
  await page.locator(".queue-row").filter({ hasText: "3 条同源记录" }).click();
  await expect(page.locator("#detail")).toContainText("归档未记录非交互进程退出码。");
