...
export async function assertBoundedDiagnosticEndpoints({ baseUrl, fs, diagnosticEvents, sessionId, requestJson, sessionDir }) {
  ...
  const nextEvents = await requestJson(baseUrl, `/api/sources/local/query/sessions/${sessionId}/events?limit=2&cursor=${diagnosticEvents.body.page.nextCursor}`);
  ...
  const diagnosticScanLimit = await requestJson(
    baseUrl,
    `/api/sources/local/query/sessions/${sessionId}/events?limit=1&cursor=10000`,
  );
  ...
  const sourceEvent = await requestJson(
    baseUrl,
    `/api/sources/local/sessions/${sessionId}/events/10000`,
  );
  ...
  const excessiveEvent = await requestJson(
    baseUrl,
    `/api/sources/local/sessions/${sessionId}/events/10001`,
  );
