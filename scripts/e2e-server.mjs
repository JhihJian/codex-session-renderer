...
await writeFile(path.join(harnessLabRoot, "registry.json"), `${JSON.stringify({
  ...
  candidates: [
    ...
    ...[40, 41, 42].map((eventIndex) => ({ id: `C-legacy-${eventIndex}`, archiveId: "A-harness", sourceHash: "a".repeat(64), detectorId: "provider-error-exit-status", detectorVersion: "1", status: "candidate", observation: "会话记录了 provider 错误，需验证非交互模式的退出状态。", suspectedRule: "最终 assistant 为 error 时，非交互输出必须返回非零退出码。", detection: { errorMessage: "400: Unsupported value: minimal", provider: "local-sub2api", model: "gpt-5.6-sol", exitStatusEvidence: "absent" }, eventIndex, eventType: "message", createdAt: "2026-08-04T08:00:00.000Z", updatedAt: "2026-08-04T08:00:00.000Z" })),
  ],
