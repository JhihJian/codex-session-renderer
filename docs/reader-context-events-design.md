...
每轮的用户和助手内容之后、上下文事件之前按可用数据显示轮次结束指标：最后一条可证明的上下文占用、该轮可证明的生成 Token 总数，以及工具与 LLM 可关联区间并集形成的实际执行时长。缺少某项原始记录时不渲染该指标；LLM 时间由事件边界推算时保留“估算”标记，不以轮次墙钟时长或会话总量代替。
...
桌面正文左侧提供固定高度的极简轮次时间轴。每个根会话轮次对应一个可点击节点，节点只用于定位现有正文 `compact-turn`，不复制工具调用、耗时、状态机或原始事件。

每个节点之后的细线代表该轮记录时长。优先使用详情 timing 中对应轮次的 `durationMs`，缺失时使用正文轮次的首尾记录时间；仍缺失或为零时使用统一的最小可见权重。时间轴在固定可视高度内按这些权重分配线段，因此短轮次不会消失，长轮次获得更长距离。中性灰色表示普通轮次，琥珀色表示含上下文压缩，青绿色表示含内嵌子代理调用；两种信号同时出现时以并列细线表达，不混成第三种颜色，也不表示成功或失败。当前阅读轮次使用蓝色节点和 `aria-current="location"`。

轮次不超过 24 时逐个展示；超过时，连续且没有压缩或子代理信号的普通轮次按最多 5 轮聚合，有信号的轮次始终单独展示。导航复用正文滚动定位，并由正文滚动位置更新当前节点。移动端隐藏该导航，并移除根正文为桌面时间轴保留的缩进，不增加新的常驻工具栏。
