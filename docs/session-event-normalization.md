...
完整详情额外返回 `timing` 投影，用于诊断页展示会话墙钟时长、等待输入、实际运行、分类投入、并行关系和数据质量。`timing.session.durationMs` 是首个有效时间点到最后有效时间点的墙钟区间。`waitingForInputMs` 只统计上一轮最后一次助手回复到下一轮有效用户消息之间的区间，原始记录没有键盘输入开始事件时不会猜测等待时间。`activeRunMs` 是工具和 LLM 可关联区间的时间并集，不把没有可靠归因的事件间隔计为运行。子代理执行不单独计时，其内部活动通过 LLM 与工具区间体现。`executionComposition` 将这些区间切成互斥的单类或并行组合段，段时长之和严格等于 `activeRunMs`，用于单一的组合条形图。两类区间可能并行，单类 `coverageMs` 都以实际运行时长为分母，比例不构成可相加的组成部分；页面同时显示重叠时长和并行峰值。没有完整起止边界的工具或事件只进入数据质量摘要，不伪造时长。
