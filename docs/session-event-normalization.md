...
同一 `sourceId + sessionId` 的展示标题由已验证的 SQLite `threads.title` 优先确定，适用于 recent/all/history 列表、详情、Markdown、任务归档和会话查询。文件或轻量索引回退已经完成路径与 ID 校验后，才会把当前受列表上限约束的候选 ID 作为一次有界 SQLite 查询输入；SQLite 返回的非空标题可以补齐文件回退标题，但不会据此新增会话、路径或正文。
