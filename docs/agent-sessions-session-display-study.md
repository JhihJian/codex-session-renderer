...
本文保留的是一次历史学习记录，其中“推荐落地方案”反映当时的设计方向。当前 `codex-session-renderer` 主界面已经调整为精简 / Audit / Raw 三个主视图：精简用于日常阅读，Audit 用于证据链复核，Raw 用于原始事件诊断。Trace 仍是服务端内部执行树和外部查询模型的一部分，用于支撑 Audit、Review Dock 和 API；主界面入口由精简 / Audit / Raw 承担。

阅读本文时可按下面口径理解旧方案：

- “阅读 / 精简 / Trace”是历史阶段的界面结构；当前主视图为“精简 / Audit / Raw”。
- “新增 Raw JSON 主视图”已经落地为 Raw 主视图，并保留按需读取完整事件 payload 的诊断入口。
- “Trace 视图”相关建议现在主要转化为 Audit 执行链、Review Dock 关系页和外部查询 API 的内部能力。
- Terminal 线性执行现场仍可作为未来增强方向，但应与当前 Audit 执行链复用同一套 turn/item 和工具摘要模型。
...
- Raw 已作为主视图落地，展示当前会话的事件摘要列表，并支持按需拉取完整事件。
- Review Dock 来源页继续承担单个事件、Audit 节点和选中对象的局部调试。
...
- 精简、Audit、Raw 以及未来可能新增的 Terminal 视图应共享搜索词，并各自维护自己的定位模型。
...
| Raw JSON 主视图 | 已作为主视图落地 | 继续优化事件摘要、pretty/raw 切换和按需完整 payload 读取 |
| 角色跳转 | 主要靠内容搜索、Audit 和 Review Dock | 增加用户/工具/错误计数与上下跳转 |
...
后续新增 Terminal 时，应作为执行现场补充视图接入现有 Review Dock；Trace 继续作为内部/外部查询模型支撑执行层级，默认主入口保持精简 / Audit / Raw。
